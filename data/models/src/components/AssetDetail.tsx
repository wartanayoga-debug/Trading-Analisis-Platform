  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (indicators.rsi > 70) {
        setAlertStatus(
          `⚠️ Peringatan (RSI Overbought ${indicators.rsi.toFixed(1)}): Waspada Tekanan Jual!`,
        );
      } else if (indicators.rsi < 30) {
        setAlertStatus(
          `⚠️ Peringatan (RSI Oversold ${indicators.rsi.toFixed(1)}): Sinyal Agresif Beli!`,
        );
...
`;
  ...
  const candMin =
    activeCandles.length > 0
      ? Math.min(...activeCandles.map((c) => c.low))
      : 100;
  const candMax =
    activeCandles.length > 0
      ? Math.max(...activeCandles.map((c) => c.high))
      : 200;

  // Forecast scaling
  const forecastCount = asset.estimatedFutureCandles?.length || 0;
  const totalPointsDivisor = Math.max(
    1,
    activeCandles.length - 1 + Math.max(0, forecastCount),
  );
  ...
  const selectedIndex =
    hoveredIndex !== null ? hoveredIndex : activeCandles.length - 1;
  ...
  const fastCoords = activeCandles
    .map((c, idx) => {
      ...
      const emaApprox =
        idx === 0
          ...
          : activeCandles
              ...
              .reduce((s, curr) => s + curr.close, 0) / Math.min(idx + 1, 5);
      return `${cx},${getCandleY(emaApprox)}`;
    })
    .join(" ");

  const slowCoords = activeCandles
    .map((c, idx) => {
      const cx = plotLeft + (idx / totalPointsDivisor) * plotWidth;
      ...
      const emaApprox =
        idx === 0
          ...
          : activeCandles
              ...
              .reduce((s, curr) => s + curr.close, 0) / Math.min(idx + 1, 10);
      return `${cx},${getCandleY(emaApprox)}`;
    })
    .join(" ");
  ...
  const localRsiData = activeCandles.map((c, idx) => {
    ...
    const offset =
      idx % 5 === 0 ? 8 : idx % 3 === 0 ? -10 : idx % 4 === 0 ? 5 : -4;
    const calculated =
      50 + (baseRsi - 50) * (idx / activeCandles.length) + offset;
  ...
  const localMacdData = activeCandles.map((c, idx) => {
    ...
    const factor =
      Math.sin(idx / 4) * 0.4 + baseHist * (idx / activeCandles.length);
    ...
    <div className="bg-gray-950 border border-gray-800 rounded-lg p-6 relative">
      ...
      <div className="mb-6">
        <span className="text-[10px] uppercase font-mono px-2 py-0.5 rounded bg-gray-900 border border-gray-800 text-gray-400">
          {asset.assetClass === "IDX"
            ? "Indonesian Stock Exchange (IDX) • Scalping Mode"
            : "Cryptocurrency Market • Scalping Mode"}
        </span>
        <h2 className="text-2xl font-display font-medium text-gray-100 flex items-center gap-2 mt-2">
          {asset.ticker}
          <span className="text-sm text-gray-500 font-mono font-normal">
            ({asset.name})
          </span>
        </h2>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-2">
          ...
          <div className="flex flex-col sm:flex-row items-end sm:items-center gap-3">
            <span
              className={`text-[10px] font-mono tracking-wider ${isAlerting ? "text-amber-400 animate-pulse" : "text-gray-500"}`}
            >
              {alertStatus}
            </span>
            <button
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="bg-gray-900 border border-gray-850 rounded-lg p-5">
              <div className="flex justify-between items-center mb-3">
                <h3 className="text-xs font-mono font-bold uppercase tracking-wider text-cyan-400">
                  {lang === "ID"
                    ? `📊 Papan Chart & Manual Analisis (${timeframe.toUpperCase()})`
                    : `📊 Technical Candlestick Pane (${timeframe.toUpperCase()})`}
                </h3>
                <span className="text-[10px] font-mono text-gray-500">
                  {lang === "ID"
                    ? "Geser kursor untuk info bar historis"
                    : "Hover columns to inspect state logs"}
                </span>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-6 gap-2 mb-4 bg-gray-950 p-2.5 rounded border border-gray-850 font-mono text-[11px] text-gray-400">
                <div>
                  ...
                  <span className="text-gray-200 font-bold block">
                    {selectedCandle
                      ? new Date(selectedCandle.time).toLocaleTimeString(
                          undefined,
                          {
                            hour: "2-digit",
                            minute: "2-digit",
                ...
                <div>
                  ...
                  <span className="text-cyan-400 font-bold block">
                    {selectedCandle
                      ? selectedCandle.open.toLocaleString(undefined, {
                ...
                <div>
                  ...
                  <span className="text-emerald-400 font-bold block">
                    {selectedCandle
                      ? selectedCandle.high.toLocaleString(undefined, {
                          maximumFractionDigits:
                            asset.assetClass === "CRYPTO" ? 4 : 0,
                        })
                      : "N/A"}
                ...
                <div>
                  ...
                  <span className="text-rose-400 font-bold block">
                    {selectedCandle
                      ? selectedCandle.low.toLocaleString(undefined, {
                          maximumFractionDigits:
                            asset.assetClass === "CRYPTO" ? 4 : 0,
                        })
                      : "N/A"}
                ...
                <div>
                  ...
                  <span className="text-cyan-400 font-bold block">
                    {selectedCandle
                      ? selectedCandle.close.toLocaleString(undefined, {
                          maximumFractionDigits:
                            asset.assetClass === "CRYPTO" ? 4 : 0,
                        })
                      : "N/A"}
              ...
              <div className="relative overflow-visible">
                ...
                >
                  ...
                  {activeCandles.map((c, idx) => {
                    const cx =
                      plotLeft + (idx / totalPointsDivisor) * plotWidth;
                    ...
                    const bodyW = Math.max(
                      3,
                      Math.min(8, plotWidth / activeCandles.length - 2),
                    );
                  ...
                  {asset.estimatedFutureCandles?.length > 0 &&
                    (() => {
                      ...
                      const lastX =
                        plotLeft +
                        ((activeCandles.length - 1) / totalPointsDivisor) *
                          plotWidth;
                      const pathPts = [
                      ...
                      asset.estimatedFutureCandles.forEach((price, i) => {
                        pathPts.push(
                          `${lastX + (i + 1) * spacing},${getCandleY(price)}`,
                        );
                      });
                      return (
                        <g>
                          ...
                          <text
                            x={lastX + spacing * 2}
                            y={getCandleY(asset.estimatedFutureCandles[2]) - 10}
                            fill="#e0e7ff"
                  ...
                  <text
                    ...
                    y={222}
                    fill="#df972c"
                    className="text-[6.5px] font-mono select-none"
                    opacity={0.7}
                  >
                    OB 70
                  </text>
                  ...
                  {localMacdData.map((d, i) => {
                    ...
                    const barW = Math.max(
                      2,
                      plotWidth / activeCandles.length - 3,
                    );
                    return (
                      <line
                        key={i}
                  ...
                  {hoveredIndex !== null &&
                    (() => {
                      const cx =
                        plotLeft +
                        (hoveredIndex / totalPointsDivisor) * plotWidth;
                      return (
                        <line
                          x1={cx}
                          y1={10}
          ...
          {/* TERMINAL DASHBOARD INTEGRATION (TradingView, Liquidity heatmap, Orderbook, Regime etc) */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-6">
            ...
            <div className="lg:col-span-7 flex flex-col gap-5">
              ...
              <div className="bg-gray-900 border border-purple-950/40 rounded-lg p-5">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                  <div>
                    <h3 className="text-xs font-mono font-bold uppercase tracking-wider text-purple-400 flex items-center gap-1.5 animate-pulse">
                      <Cpu className="w-4 h-4" /> LM Studio Sentiment News
                      Analyzer
                    </h3>
                    <p className="text-[11px] text-gray-400 mt-0.5 font-sans leading-normal">
                      Menganalisis sentimen media dan aliran informasi pasar
                      secara lokal via LM Studio
                    </p>
                  </div>
                  ...
                  <div className="bg-gray-950/60 border border-gray-850 p-4 rounded flex gap-3 text-xs text-gray-400 mb-4 items-start font-sans">
                    ...
                    <div className="space-y-1 leading-normal">
                      <p className="text-gray-200 font-bold">
                        Infrastruktur LM Studio Belum Terdeteksi
                      </p>
                      <p className="text-[11px] text-gray-450 leading-relaxed">
                        Aplikasi mendeteksi bahwa port local server LM Studio
                        Anda belum diaktifkan. Anda masih dapat menjalankan
                        sentimen, tetapi untuk menjalankan sentimen real-time
                        pastikan:
                      </p>
                      <ul className="list-disc pl-4 text-[10px] space-y-1 text-gray-500 font-mono">
                        ...
                        <li>
                          Sisi kiri klik ikon{" "}
                          <strong>&lt;/&gt; (Developer/Local Server)</strong>.
                        </li>
                        <li>
                          Nyalakan port <strong>1234</strong> dan klik{" "}
                          <strong>Start Server</strong>.
                        </li>
                        <li>
                          Pastikan CORS diizinkan agar browser diizinkan menarik
                          data.
                        </li>
                      </ul>
                    </div>
                  ...
                  <div>
                    {lmResponse ? (
                      ...
                      <span className="text-gray-500 font-mono italic">
                        Menunggu pemicuan analisis sentimen berita lokal LM
                        Studio...
                      </span>
                    )}
                  </div>
                  <p className="text-[10px] text-gray-500 font-mono mt-4 leading-normal">
                    🤖 Sentimen lokal server-side (RSI:{" "}
                    {indicators?.rsi.toFixed(1) || "50.0"}, ADX:{" "}
                    {indicators?.adx.toFixed(1) || "20.0"}), optimal scalp
                    limits, & model calibration.
                  </p>
                </div>
                ...
                >
                  <Play
                    className={`w-3.5 h-3.5 ${isLmLoading ? "animate-spin" : ""}`}
                  />
                  {isLmLoading
                    ? "Menganalisa Berita via LM Studio..."
                    : "Jalankan Analisis Sentimen Berita lokal LM Studio"}
                </button>
              </div>
            </div>
              ...
              <div className="bg-gray-950 border border-gray-800 rounded-lg p-5">
                ...
                <div className="space-y-4 font-mono text-xs">
                  ...
                  <div className="flex items-center justify-between py-2 border-b border-gray-900">
                    ...
                    <span
                      className={`font-bold flex items-center gap-1 ${labelColor}`}
                    >
                      {isBull ? (
                  ...
                  <div className="flex items-center justify-between py-2 border-b border-gray-900">
                    ...
                    <span className="font-bold text-gray-300 flex items-center gap-1">
                      ...
                      {asset.entryZone.min.toLocaleString(undefined, {
                        maximumFractionDigits:
                          asset.assetClass === "CRYPTO" ? 2 : 0,
                      })}{" "}
                      -{" "}
                      {asset.entryZone.max.toLocaleString(undefined, {
                        maximumFractionDigits:
                          asset.assetClass === "CRYPTO" ? 2 : 0,
                      })}
                    </span>
                  ...
                  <div className="flex items-center justify-between py-2 border-b border-gray-900">
                    ...
                    <span className="font-bold text-emerald-400 flex items-center gap-1">
                      ...
                      {asset.takeProfit.toLocaleString(undefined, {
                        maximumFractionDigits:
                          asset.assetClass === "CRYPTO" ? 2 : 0,
                      })}
                    </span>
                  ...
                  <div className="flex items-center justify-between py-2 border-b border-gray-900">
                    ...
                    <span className="font-bold text-rose-400 flex items-center gap-1">
                      ...
                      {asset.stopLoss.toLocaleString(undefined, {
                        maximumFractionDigits:
                          asset.assetClass === "CRYPTO" ? 2 : 0,
                      })}
                    </span>
                  ...
                  <div className="flex items-center justify-between py-2 border-b border-gray-900">
                    <span className="text-gray-500">
                      {t.thesisInvalidation}
                    </span>
                    <span className="font-bold text-amber-500 flex items-center gap-1">
                      <AlertTriangle className="w-3.5 h-3.5 text-amber-500" />
                      {asset.invalidationLevel.toLocaleString(undefined, {
                        maximumFractionDigits:
                          asset.assetClass === "CRYPTO" ? 2 : 0,
                      })}
                    </span>
              ...
              <div className="bg-gray-900/30 p-4 border border-gray-900 rounded-lg text-xs font-mono">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span
                      className={`font-bold ${asset.manipulationWarning ? "text-rose-400" : "text-emerald-400"}`}
                    >
                      {asset.manipulationWarning
                        ? t.alertAbnormalVolume
                        : t.normalTransactions}
                    </span>
                  </div>
                  <div className="flex justify-between">
              ...
              <div className="bg-gray-950 border border-gray-800 rounded-lg p-5">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs font-mono">
                  <div className="bg-gray-900/40 p-3 rounded border border-gray-905">
                    ...
                    <span
                      className={`text-base font-bold ${indicators && indicators.rsi > 70 ? "text-amber-500" : indicators && indicators.rsi < 30 ? "text-cyan-400" : "text-gray-300"}`}
                    >
                  ...
                  <div className="bg-gray-900/40 p-3 rounded border border-gray-905">
                    ...
                    <span className="text-base font-bold text-gray-300 truncate block">
                      {indicators
                        ? `${Math.round(indicators.emaFast)}/${Math.round(indicators.emaSlow)}`
                        : "N/A"}
                    </span>
                  </div>
                  <div className="bg-gray-900/40 p-3 rounded border border-gray-905">
                    ...
                    <span
                      className={`text-base font-bold ${indicators && indicators.macdHist >= 0 ? "text-emerald-400" : "text-rose-400"}`}
                    >
